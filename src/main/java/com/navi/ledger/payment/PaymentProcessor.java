...
public class PaymentProcessor {
    ...
    public List<Payment> updatePayments(List<Payment> paymentList, double lumSumAmount, int afterEmiCount, int emiAmount, double totalAmount) {
        if(afterEmiCount == 0) {
            paymentList = firstPaymentProcess(paymentList,totalAmount,lumSumAmount, emiAmount);
        } else {
            for (int i = paymentList.size(); i < afterEmiCount; i++) {
                Payment prevPayment = paymentList.get(i - 1);
                paymentList.add(paymentProcess(prevPayment, emiAmount, totalAmount));
            }
            if (paymentList.size() == afterEmiCount) {
                Payment prevPayment = paymentList.get(paymentList.size() - 1);
                paymentList.add(finalPaymentProcess(prevPayment, emiAmount, lumSumAmount, totalAmount));
            }
        }
        return paymentList;
    }

    public List<Payment> firstPaymentProcess(List<Payment> paymentList, double totalAmount, double lumSumAmount, int emiAmount ) {
    ...
    public Payment finalPaymentProcess(Payment prevPayment, int emiAmount, double lumSumAmount, double totalAmount) {
    ...
    public Payment paymentProcess(Payment prevPayment, int emiAmount, double totalAmount) {
