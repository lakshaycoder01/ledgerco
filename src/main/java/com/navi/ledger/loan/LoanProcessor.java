...
public class LoanProcessor {
    ...
    private void loanMapUpdation(Loan loan) {
        if(Util.userLoanMapping().containsKey(customerName)) {
            Map<String, Loan> existingMap = Util.userLoanMapping().get(customerName);
            existingMap.put(bankName, loan);
            Util.userLoanMapping().put(customerName, existingMap);
        } else {
            Map<String,Loan> map = new HashMap<>();
            map.put(bankName,loan);
            Util.userLoanMapping().put(customerName,map);
        }
    }
